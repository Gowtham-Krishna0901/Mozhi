...
async function signUp(email, password, role, fullName) {
  ...
  // Give the new account a profile row right away — role and
  // name are set now, even though the account can't sign in
  // until the email is verified (if verification is enabled
  // on this Supabase project).
  const { error: profileError } = await supabaseClient
    .from("profiles")
    .upsert({ id: newUser.id, role, full_name: fullName });
  ...
  if (data.session) {
    // Email verification isn't required on this project, so
  ...
  // Email verification is required — Supabase has already sent
  // the verification email. They can't sign in until they click
  // the link in it.
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (!profile || !profile.role) {
    showAuthBanner("Email verified! Please sign in below.", "success");
    await supabaseClient.auth.signOut();
    return;
  }
