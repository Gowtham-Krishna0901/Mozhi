...
function renderCategoryChips() {
  ...
  wrap.innerHTML = CATEGORIES.map(
    (cat) => `
    <button class="chip ${cat.id === activeCategoryId ? "active" : ""}" type="button" data-cat="${cat.id}">
      ${cat.emoji} ${cat.label}
    </button>
  `
  ).join("");
    ...
    .map(
      ...
      <div class="manage-row" data-id="${p.id}">
        ...
        <div class="manage-row-actions">
          <button type="button" data-action="edit" aria-label="Edit phrase">✏️</button>
          <button type="button" data-action="delete" class="delete-btn" aria-label="Delete phrase">🗑️</button>
        </div>
      </div>
    `
    )
...
function beginEdit(row, id) {
  ...
  row.querySelector(".manage-row-actions").innerHTML = `
    <button type="button" data-action="save" aria-label="Save phrase">✅</button>
    <button type="button" data-action="cancel" aria-label="Cancel edit">✖️</button>
  `;
