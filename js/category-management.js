...
function renderCategoryChips() {
  ...
  wrap.innerHTML = CATEGORIES.map(
    ...
    <button class="chip ${cat.id === activeCategoryId ? "active" : ""} ${cat.id === "emergency" ? "emergency" : ""}" type="button" data-cat="${cat.id}">
      ...
      <span class="chip-label">${categoryLabel(cat.id)}</span>
    </button>
  `
  ).join("");
...
function wireAddForm() {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    if (error) {
      showManageMessage(t("msg_add_failed"), true);
      return;
    }

    input.value = "";
    showManageMessage(t("msg_phrase_added"));
    await loadManageList();
  });
}
...
async function loadManageList() {
  ...
  if (error) {
    listEl.innerHTML = `<p class="empty-state">${t("empty_load_failed_manage")}</p>`;
    return;
  }

  if (!phrases || phrases.length === 0) {
    listEl.innerHTML = `<p class="empty-state">${t("empty_no_phrases_category")}</p>`;
    return;
  }
...
async function saveEdit(row, id, newText) {
  ...
  if (error) {
    showManageMessage(t("msg_save_failed"), true);
    return;
  }

  showManageMessage(t("msg_phrase_updated"));
  await loadManageList();
}

async function deletePhrase(row, id) {
  const confirmed = window.confirm(t("confirm_delete_phrase"));
  if (!confirmed) return;

  const { error } = await supabaseClient.from("phrases").delete().eq("id", id);

  if (error) {
    showManageMessage(t("msg_delete_failed"), true);
    return;
  }

  showManageMessage(t("msg_phrase_deleted"));
  await loadManageList();
}
