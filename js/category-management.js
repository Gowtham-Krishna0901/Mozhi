...
function renderCategoryChips() {
  ...
  wrap.innerHTML = CATEGORIES.map(
    ...
    <button class="chip ${cat.id === activeCategoryId ? "active" : ""} ${cat.id === "emergency" ? "emergency" : ""}" type="button" data-cat="${cat.id}">
      <span class="chip-icon" aria-hidden="true">${cat.emoji}</span>
  ...
  listEl.innerHTML = phrases
    .map(
      ...
      <div class="manage-row" data-id="${p.id}">
        <span class="manage-row-icon" aria-hidden="true">${p.emoji || "💬"}</span>
        <span class="manage-row-text" data-role="display">${escapeHtml(p.text)}</span>
        <div class="manage-row-actions">
          <button type="button" class="row-action-btn edit-btn" data-action="edit" aria-label="Edit phrase">✏️</button>
          <button type="button" class="row-action-btn delete-btn" data-action="delete" aria-label="Delete phrase">🗑️</button>
        </div>
      </div>
    `
    )
...
function beginEdit(row, id) {
  ...
  row.querySelector(".manage-row-actions").innerHTML = `
    <button type="button" class="row-action-btn save-btn" data-action="save" aria-label="Save phrase">✅</button>
    <button type="button" class="row-action-btn cancel-btn" data-action="cancel" aria-label="Cancel edit">✖️</button>
  `;
