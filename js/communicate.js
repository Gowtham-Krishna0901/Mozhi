...
async function initCategoryPage(listEl) {
  ...
  if (titleEl) {
    titleEl.innerHTML = `<span class="page-title-icon" aria-hidden="true">${getCategoryIcon(category.id, 22)}</span>${category.label}`;
  }
...
async function loadPhrases(category, listEl) {
  ...
  let effectivePhrases = phrases;

  if (error || !phrases) {
    // Fetch failed — most likely offline. Fall back to the last
    // successful copy of this category's phrases so the patient
    // still has tappable buttons instead of an error screen.
    const cached = getCachedPhrasesForCategory(category.id);
    if (cached && cached.length > 0) {
      effectivePhrases = cached;
    } else {
      listEl.innerHTML = `<p class="empty-state">Couldn't load phrases. Please try again.</p>`;
      return;
    }
  } else {
    cachePhrasesForCategory(category.id, phrases);
  }

  if (!effectivePhrases || effectivePhrases.length === 0) {
    listEl.innerHTML = `<p class="empty-state">No phrases here yet. Ask a family member to add some.</p>`;
    return;
  }

  listEl.innerHTML = effectivePhrases
    .map(
      (p) => `
...
async function sendPhrase(category, btn) {
  ...
  // the patient gets instant audio feedback even on a slow (or
  // no) connection, and it happens regardless of send outcome.
  speakPhrase(phraseText);

  const result = await sendPatientMessage(currentPatientUser.id, category.id, phraseText);

  btn.classList.remove("sending");

  if (!result.ok) {
    showToast("Couldn't send. Please try again.", true);
    return;
  }

  if (result.queued) {
    showToast(`Saved — will send once you're back online: "${phraseText}"`);
  } else {
    showToast(`Message Sent: "${phraseText}"`);
  }
