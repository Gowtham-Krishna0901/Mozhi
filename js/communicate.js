...
function renderCategoryGrid(grid) {
  grid.innerHTML = CATEGORIES.map((cat) => `
    <button class="category-tile ${cat.id === "emergency" ? "emergency" : ""}" type="button"
      ...
      <span class="tile-icon" aria-hidden="true">${cat.emoji}</span>
...
async function initCategoryPage(listEl) {
  ...
  if (titleEl) titleEl.textContent = `${category.emoji} ${category.label}`;
  ...
  listEl.innerHTML = phrases
    .map(
      ...
      <button class="phrase-btn" type="button" data-phrase-id="${p.id}" data-phrase-text="${escapeHtml(p.text)}">
        <span class="phrase-emoji" aria-hidden="true">${p.emoji || category.emoji}</span>
...
function showToast(message, isError) {
  ...
  toast.innerHTML = `<span aria-hidden="true">${isError ? "⚠️" : "✅"}</span><span>${message}</span>`;
